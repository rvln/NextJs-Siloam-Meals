...
export default function ManageFood() {
    ...
    async function fetchFood() {
      try {
        ...
        const cleanedFoods: Makanan[] = dataFromApi.map((apiItem) => ({
          ...
          createdBy: apiItem.user ? apiItem.user.namaUser : "Pengguna Dihapus", // Perbaikan di sini
          utamaDari: apiItem.utamaDari
            ? apiItem.utamaDari.map((k) => ({
                id: k.idMakanan,
                nama: k.namaMakanan,
                jenis: k.jenis,
              }))
  ...
  const filteredFoodItems = (
    activeTab === "utama" ? makananUtama : makananPendamping
  ).filter((food) =>
    food.nama.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleCreateFood = async (formData: FormData) => {
    try {
      ...
      const newFood: Makanan = {
        ...
        createdBy: created.user ? created.user.namaUser : "Pengguna Dihapus", // Perbaikan di sini
  ...
  const handleSaveFood = async (id: number, formData: FormData) => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/makanan/${id}`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` }, // Content-Type dihapus agar browser menentukannya sendiri
          body: formData,
        }
      );
      if (!res.ok) {
      ...
      const updated: Makanan = {
        ...
        jenis: saved.jenis,
        gambar: saved.gambar,
        createdBy: saved.user ? saved.user.namaUser : "Pengguna Dihapus", // Perbaikan di sini
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/makanan/${foodToDelete.id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
              ...
              <div className="p-6 border-b border-gray-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-sm text-gray-500">
                    {filteredFoodItems.length} makanan
                  </span>
                </div>
          ...
          <div className="mt-8 lg:mt-0 lg:col-span-8 xl:col-span-9">
            {isCreating ? (
              ...
              isEditing ? (
                ...
                <FoodDetailView
                  food={selectedFood}
                  onEdit={() => handleEditFood(selectedFood)}
                />
              )
            ) : (
              <div className="bg-gray-200 rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                ...
                <p className="text-gray-500">
                  Pilih makanan dari daftar di sebelah kiri untuk melihat detail
                  dan mengelola data makanan.
                </p>
              </div>
            )}
          </div>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong>{foodToDelete?.nama}</strong>? Tindakan ini tidak dapat
              dibatalkan.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setFoodToDelete(null)}>
              Batal
            </Button>
